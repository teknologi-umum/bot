...
test("should throw error on comparing dates", () => {
  assert.throws(
    () => new Temporal(DATE).compare(),
    "Come on bro, niat ga sih?"
  );
...
test("should be able to format date", () => {
  ...
  assert.is(
    new Temporal(DATE).formatDate("en-US", "UTC", true, false),
    "December 17, 1995 at 3:24:00 AM UTC"
  );
  assert.is(
    new Temporal(DATE).formatDate("en-US", "UTC", true, true),
    "Sunday, December 17, 1995 at 3:24:00 AM UTC"
  );
  assert.is(
    new Temporal(DATE).formatDate("en-US", "UTC", false, true),
...
test("should throw error on add duration", () => {
  assert.throws(
    () => new Temporal(DATE).add(false),
    "duration must be a type of number"
  );
